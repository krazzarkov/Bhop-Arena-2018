...
{
    public static List<CPMPlayer> AllPlayers; //List of all players currently connected
    
    //Helper property to easily get the local player, as opposed to all other players connected to the server
    public static CPMPlayer LocalPlayer
    {
        get { return AllPlayers.Find(player => player.isLocalPlayer); }
    }
...
//
    ...
    private void Start()
    {
        ...
        {
            //Enable the camera if this is the local player and disable it if it isn't
    ...
    private void Update()
    {
        ...
        //Only allow movement if this is the local player
        if (!isLocalPlayer)
            return;
    ...
    */
    ...
    public void RpcKnockback(Vector3 direction, float force)
    {
        ...
        float playerMass = 5f;
        playerVelocity += force * direction.normalized / playerMass;
        _controller.Move(playerVelocity * Time.deltaTime);
    }
}